...
export default function InstagramShowcase({ onOpenLightbox }) {
  ...
  return (
    <section className="py-14 sm:py-20 bg-[#2B1C19] border-t border-[#D4AF37]/20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            ...
            <div className="flex items-start space-x-4">
              ...
              <div>
                ...
                <p className="mt-1 text-xs sm:text-sm text-[#D4AF37] max-w-xl font-normal leading-relaxed">
        ...
        <div key={activeCategory} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 animate-slide-right-pop">
          {displayedPosts.map((post, idx) => (
            <div
              key={post.id}
              className="group bg-[#3E2723] border border-[#D4AF37]/20 hover:border-[#D4AF37]/50 rounded-xl overflow-hidden shadow-xl transition-all duration-300 flex flex-col justify-between cursor-pointer reveal-up"
            ...
            >
              ...
              <div className="relative aspect-square overflow-hidden bg-black/40">
                ...
                <div className="absolute inset-0 bg-[#2B1C19]/85 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-5 text-white">
                  ...
                  <div className="flex items-center justify-between text-xs font-mono pt-3 border-t border-[#D4AF37]/20">
              ...
              <div className="p-4 bg-[#2B1C19] border-t border-[#D4AF37]/20 flex items-center justify-between text-xs">
                <div className="flex items-center space-x-2 text-[#D4AF37] font-mono text-[11px]">
        ...
        {filteredPosts.length > initialLimit && (
          <div className="mt-8 text-center">
            <button
              ...
              className="px-6 py-3 rounded-xl bg-[#3E2723] hover:bg-[#4E342E] border border-[#D4AF37]/50 text-[#FAF7F0] hover:text-[#D4AF37] text-xs font-semibold uppercase tracking-[0.2em] transition-all shadow-md inline-flex items-center gap-2"
            >
        ...
        {selectedStory && typeof document !== 'undefined' && createPortal(
          ...
          >
            <div
              className="bg-[#2B1C19] border border-[#D4AF37]/50 rounded-2xl max-w-sm w-full p-4 sm:p-5 text-[#FAF7F0] shadow-2xl relative animate-pop-up my-auto flex flex-col max-h-[92vh]"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header */}
              <div className="flex items-center justify-between border-b border-[#D4AF37]/20 pb-3 flex-shrink-0">
                <div className="flex items-center space-x-2.5">
                  <div className="w-9 h-9 rounded-full p-[2px] bg-gradient-to-tr from-[#D4AF37] to-[#D4AF37] flex-shrink-0">
                    <img src={selectedStory.cover} alt="" className="w-full h-full object-cover rounded-full" />
                  </div>
                  <div>
                    <h4 className="font-bold text-sm text-[#FAF7F0] leading-tight">{selectedStory.title}</h4>
                    <span className="text-[10px] text-[#D4AF37] font-mono">{selectedStory.tag} • {selectedStory.count} Clips</span>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedStory(null)}
                  className="p-1.5 text-[#D4AF37] hover:text-[#FAF7F0] hover:bg-white/10 rounded-full transition-colors"
              ...
              <div className="relative w-full h-[54vh] max-h-[460px] rounded-xl overflow-hidden bg-black flex-shrink-0 my-3.5 shadow-lg border border-[#D4AF37]/20">
                <img
                  src={selectedStory.cover}
                  alt={selectedStory.title}
                  className="w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#2B1C19]/95 via-transparent to-black/40 p-4 flex flex-col justify-between">
                  <div className="flex justify-between items-center text-[10px] font-mono text-[#FAF7F0]/90">
                    <span className="bg-[#2B1C19]/80 px-2 py-0.5 rounded border border-[#D4AF37]/30">HYZIN ARCHIVE</span>
                    <span>{instagramProfile.handle}</span>
                  </div>
                  <div>
                    <p className="text-xs text-[#FAF7F0]/90 font-normal mb-3 leading-relaxed">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="w-full py-2.5 bg-[#D4AF37] hover:bg-[#FAF7F0] text-[#2B1C19] text-xs font-bold font-mono uppercase tracking-wider flex items-center justify-center space-x-2 rounded-lg transition-colors shadow-md"
                    >
              ...
              <div className="text-center text-[11px] text-[#D4AF37]/70 flex-shrink-0">
        ...
        <div className="mt-10 sm:mt-12 bg-[#3E2723] border border-[#D4AF37]/30 rounded-2xl p-6 sm:p-7 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-6 shadow-2xl">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-xs sm:text-sm text-[#D4AF37] font-normal leading-relaxed">
